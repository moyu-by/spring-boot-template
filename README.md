...
> **IntelliJ IDEA 用户：** 自定义配置（`jwt.*`、`file.*` 等）若在 `application.yml` 中无法正确识别（标红），有两种解决方法：
> ① 在 `application.yml` 首行加 `# yaml-language-server: $schema=https://json.schemastore.org/spring-boot.json`
> ② 关闭 IDEA → 删除项目根目录下的 `.idea` 文件夹 → 重新导入 Maven 项目
> VS Code 用户无需此操作。
